...
def stub_reply(state: AgentState) -> AgentState:
    ...
    state.draft_v1 = "[STUB] Voici une réponse générique."  # TODO: message minimal (sera remplacé plus tard)
...
def stub_ask_clarification(state: AgentState) -> AgentState:
    ...
    state.draft_v1 = "Pouvez-vous préciser votre demande ?"  # TODO: 1-2 questions génériques (sera remplacé plus tard)
...
def stub_escalate(state: AgentState) -> AgentState:
    ...
    state.actions.append({
        "type": "handoff_human",
        "summary": "Escalade demandée par le modèle (risque ou complexité).",   # TODO: résumé court pour escalade
    })
...
def stub_ignore(state: AgentState) -> AgentState:
    ...
    state.actions.append({
        "type": "ignore",
        "reason": "Hors périmètre ou Spam.",  # TODO: raison courte (ex: hors périmètre)
    })
